...
export const classJoinRequests = pgTable(
  "class_join_requests",
  {
    ...
    studentId: text("student_id")
      .notNull()
      .references(() => user.id, { onDelete: "cascade" }),
    status: pgEnum("join_request_status", ["pending", "accepted", "declined"])(
      "status",
    )
      .default("pending")
      .notNull(),
  ...
  (table) => [
    unique("class_join_request_unique").on(table.classId, table.studentId),
    index("class_join_requests_class_id_idx").on(table.classId),
    index("class_join_requests_student_id_idx").on(table.studentId),
  ],
);
...
export const classJoinRequestsRelations = relations(
  classJoinRequests,
  ({ one }) => ({
    class: one(classes, {
